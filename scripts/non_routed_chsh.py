...
The protocols we consider are as follows:
    ...
    Bob measures Z+X, Z-X and X. Key is generated when Alice and Bob both measure X (i.e., x = 0 and y = 2).
"""
...
def HAgB(MA,rho, etaA, etaB, q):
    ...
    if Bob_bins_for_key_generation == False:
...
def get_p_ideal(phip,MA,MB):
    ...
    pbb84 = pchsh ## Comment this line if you want BB84 correlations between Alice and Bob. Uncomment if you want chsh correlations between them. 

    return pbb84

def get_p_obs(pbb84,etaA,etaB,Alice_bins_for_test_rounds,Bob_bins_for_test_rounds):
    """
    ...
    for a in range(nA):
        for b in range(nB):
            for x in range(nX):
                for y in range(nY):
                    p_obs[a,b,x,y] += etaA * etaB * pbb84[a,b,x,y]

                    if Alice_bins_for_test_rounds == True:
                        p_obs[a,b,x,y] += (1 - etaA) * etaB * np.sum(pbb84[:,b,0, y]) * (1-a)
                    if Bob_bins_for_test_rounds == True:
                        p_obs[a,b,x,y] += etaA * (1 - etaB) * np.sum(pbb84[a,:, x,0]) * (1-b)
                    if Alice_bins_for_test_rounds == True and Bob_bins_for_test_rounds == True:
                        p_obs[a,b,x,y] += (1 - etaA) * (1 - etaB)*(1-a)*(1-b)

    ### Add the probabilities for 'no-click' events
    
    for x in range(nX):
        for y in range(nY):
            if Alice_bins_for_test_rounds == False and Bob_bins_for_test_rounds == False:
                p_obs[2,2,x,y] += (1 - etaA) * (1 - etaB)

            for a in range(nA):
                if Bob_bins_for_test_rounds == False:
                    p_obs[a,2,x,y] += etaA * (1-etaB) * np.sum(pbb84[a,:,x,0])
                if Alice_bins_for_test_rounds == True and Bob_bins_for_test_rounds == False:
                    p_obs[a,2,x,y] += (1 - etaA) * (1 - etaB)*(1-a)
            for b in range(nB):
                if Alice_bins_for_test_rounds == False:
                    p_obs[2,b,x,y] += (1-etaA) * etaB * np.sum(pbb84[:,b,0,y])
                if Alice_bins_for_test_rounds == False and Bob_bins_for_test_rounds == True:
                    p_obs[2,b,x,y,1] += (1 - etaA) * (1 - etaB)*(1-b)
...
def score_constraints(P, p_obs):
    ...
    for a in range(nA):
        for b in range(nB):
            for x in range(nX):
                for y in range(nY):
                    constraints += [P([a,b],[x,y]) - p_obs[a, b, x, y]]
...
def get_extra_monomials():
    ...
    ###Add some more monos
    monos += [a*a_*b*z for a in Aflat for a_ in Aflat for b in Bflat for z in ZZ] #AABZ
...
def compute_keyrate(etaB):
    ...
    p_obs = get_p_obs(pbb84, etaA, etaB,Alice_bins_for_test_rounds,Bob_bins_for_test_rounds) 
...
p_obs = get_p_obs(pbb84,1,1,Alice_bins_for_test_rounds,Bob_bins_for_test_rounds) #placeholder p_obs
...
etaAs = [1]
etaAs.reverse()
print("non-routed CHSH Bob doesn't bin")
for etaA in etaAs:
    etaBs = np.arange(0.5,0.96,0.01499).tolist()[:-1] + np.linspace(0.96,0.99,8).tolist()[:-1] + np.linspace(0.99,0.999999,3).tolist()
    etaBs.reverse()
    outputs = []
    for etaB in etaBs:
        try:
            etaB_, keyrate_, q_, ent_, err_ = compute_keyrate(etaB)
            outputs = outputs + [[etaB_, keyrate_, q_, ent_, err_]]
            print("etaA = ", etaA, "etaB = ", etaB, "keyrate = ", keyrate_)
            if keyrate_ < 0:
                break
        except:
            print("Error in computing keyrate for etaA = ", etaA, "and etaB = ", etaB)
            pass


    ### Save the outputs to a file
    np.savetxt('non_routed_chsh_bob_nobin_etaA_'+str(etaA)+'.txt', outputs)    
