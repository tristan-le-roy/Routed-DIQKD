...
def objective(ti, q):
    ...
    for a in range(A_config[0]):
        ...
        obj += M0 * (Z[a] + Dagger(Z[a]) + (1-ti)*Dagger(Z[a])*Z[a]) + ti*Z[a]*Dagger(Z[a])
...
def get_p_obs(pchsh,pbb84,etaA,etaB,Alice_bins_for_test_rounds,Bob_bins_for_test_rounds): 
    ...
    for a in range(nA):
        for b in range(nB):
            for x in range(nX):
                for y in range(2):
                    p_obs[a,b,x,y] += etaA * etaB * pchsh[a,b,x,y]

                    if Alice_bins_for_test_rounds == True:
                        p_obs[a,b,x,y] += (1 - etaA) * etaB * np.sum(pchsh[:,b,0, y]) * (1-a)
                    if Bob_bins_for_test_rounds == True:
                        p_obs[a,b,x,y] += etaA * (1 - etaB) * np.sum(pchsh[a,:, x,0]) * (1-b)
                    if Alice_bins_for_test_rounds == True and Bob_bins_for_test_rounds == True:
                        p_obs[a,b,x,y] += (1 - etaA) * (1 - etaB)*(1-a)*(1-b)
                for y in range(2,nY):
                    p_obs[a,b,x,y] += etaA * etaB * pbb84[a,b,x,y-2]

                    if Alice_bins_for_test_rounds == True:
                        p_obs[a,b,x,y] += (1 - etaA) * etaB * np.sum(pbb84[:,b,0, y-2]) * (1-a)
                    if Bob_bins_for_test_rounds == True:
        ...
        for y in range(2):
            ...
            for a in range(nA):
                if Bob_bins_for_test_rounds == False:
                    p_obs[a,2,x,y] += etaA * (1-etaB) * np.sum(pchsh[a,:,x,0])
            ...
            for b in range(nB):
                if Alice_bins_for_test_rounds == False:
                    p_obs[2,b,x,y] += (1-etaA) * etaB * np.sum(pchsh[:,b,0,y-2])
                if Alice_bins_for_test_rounds == False and Bob_bins_for_test_rounds == True:
                    p_obs[2,b,x,y] += (1 - etaA) * (1 - etaB)*(1-b)
        for y in range(2,nY):
            if Alice_bins_for_test_rounds == False and Bob_bins_for_test_rounds == False:
                p_obs[2,2,x,y] += (1 - etaA) * (1 - etaB)
            ...
            for b in range(nB):
                if Alice_bins_for_test_rounds == False:
                    p_obs[2,b,x,y] += (1-etaA) * etaB * np.sum(pbb84[:,b,0,y-2])
                if Alice_bins_for_test_rounds == False and Bob_bins_for_test_rounds == True:
                    p_obs[2,b,x,y] += (1 - etaA) * (1 - etaB)*(1-b)
...
p_obs = get_p_obs(pchsh,pbb84,1,1,Alice_bins_for_test_rounds,Bob_bins_for_test_rounds) #placeholder p_obs
...
for etaA in etaAs:
    ...
    np.savetxt('non_routed_one_basis_chsh_bb84_bin_etaA_'+str(etaA)+'.txt', outputs)    
