...
The protocols we consider are as follows:
    ...
    Distant Bob measures X and Z. Key is generated when Alice and Bob both measure 
    in the same basis (i.e., x = 0 and y = 0 or x = 1 and y = 1).
"""
...
def objective(ti, q):
    ...
    for a in range(A_config[0]):
        ...
        obj_1 += M * (Z[a] + Dagger(Z[a]) + (1-ti)*Dagger(Z[a])*Z[a]) + ti*Z[a]*Dagger(Z[a])
    ...
    for a in range(A_config[1]):
        b = (a + 1) % 2                     # (a + 1 mod 2)
        M = (1-q) * F[a] + q * F[b]         # Noisy preprocessing povm element
        obj_2 += M * (Z[a+2] + Dagger(Z[a+2]) + (1-ti)*Dagger(Z[a+2])*Z[a+2]) + ti*Z[a+2]*Dagger(Z[a+2])
...
def HAgB(MA,rho, etaA, etaBL, q):
    ...
    a00 = MA[0,0]
    b20 = MA[0,0] #Bob's measurement is the same as Alice's for key generation
    ...
    if BobL_bins_for_key_generation == False:
        ...
        qmarg = [qb0,qb1,qb2]
        cond_ent_1 = cond_ent(qjoint, qmarg)
    ...
    # Noiseless measurements
    a00 = MA[0,1]
    b20 = MA[0,1] #Bob's measurement is the same as Alice's for key generation
    ...
    if BobL_bins_for_key_generation == False:
        ...
        cond_ent_2 = cond_ent(qjoint, qmarg)
    else:
        # Bob has inefficient measurement and bins
        B20 = etaBL * b20 + (1-etaBL) * id
        B21 = id - B20
    ...
    return 0.5*cond_ent_1 + 0.5*cond_ent_2
...
BobL_bins_for_test_rounds = False
...
Z = ncp.generate_operators('Z1', A_config[0], hermitian=False) + ncp.generate_operators('Z2', A_config[1], hermitian=False)
...
outputs_of_outputs = []
etaAs = [0.94,0.92,0.90]
etaAs.reverse()
for etaA in etaAs:
    etaBS = etaA; 
    etaBLs = np.arange(0.5,0.96,0.02).tolist()[:-1] + np.linspace(0.96,0.99,6).tolist()[:-1] + np.linspace(0.99,0.999999,3).tolist()
    etaBLs.reverse()
    outputs = []
    for etaBL in etaBLs:
        try:
    ...
    np.savetxt('two_bases_bb84_bobL_nobin_vis_1_etaA_'+str(etaA)+'.txt', outputs)    
