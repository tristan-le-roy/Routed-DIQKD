...
for etaA in etaAs:
    etaBS = etaA; 
    etaBLs = np.arange(0.5,0.74,0.05).tolist() + np.linspace(0.96,0.99,8).tolist()[:-1] + np.linspace(0.99,0.999999,3).tolist()
    etaBLs.reverse()
    outputs = []
    for etaBL in etaBLs:
        try:
    ...
    np.savetxt('chsh_bin_vis_0.99_etaA_'+str(etaA)+'.txt', outputs)    
